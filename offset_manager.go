...
type PartitionOffsetManager interface {
	// Offset returns the current offset according to the manager; this value has not necessarily
	// been flushed to the cluster yet.
	Offset() int64
	// SetOffset sets the current offset according to the manager; this value (or a subsequent update)
	// will eventually be flushed to the cluster based on configuration.
	SetOffset(offset int64)

	// Metadata returns the current metadata according to the manager; this value has not necessarily
	// been flushed to the cluster yet.
	Metadata() string
	// SetMetadata sets the current metadata according to the manager; this value (or a subsequent update)
	// will eventually be flushed to the cluster based on configuration.
...
func (pom *partitionOffsetManager) selectBroker() error {
	...
	if err = pom.parent.client.RefreshCoordinator(pom.parent.conf.Consumer.Group); err != nil {
		return err
	}

	if broker, err = pom.parent.client.Coordinator(pom.parent.conf.Consumer.Group); err != nil {
		return err
	}
...
func (pom *partitionOffsetManager) fetchInitialOffset(retries int) error {
	...
	request.Version = 1
	request.ConsumerGroup = pom.parent.conf.Consumer.Group
...
func (bom *brokerOffsetManager) constructRequest() *OffsetCommitRequest {
	r := &OffsetCommitRequest{
		Version:       1,
		ConsumerGroup: bom.parent.conf.Consumer.Group,
	}
	for s := range bom.subscriptions {
		s.lock.Lock()
